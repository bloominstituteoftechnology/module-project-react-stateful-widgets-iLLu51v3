...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(programmer => programmer.id == select).name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: select ? 'gold': 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setSelect(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          select ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
